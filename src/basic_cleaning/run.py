...
def go(args):
    ...
    logger.info(
        "Dataset price outliers removal outside range: %s-%s", min_price, max_price
    )

    dataframe["last_review"] = pd.to_datetime(dataframe["last_review"])
    logger.info("Dataset last_review data type fix")

    idx = dataframe["longitude"].between(-74.25, -73.50) & dataframe[
        "latitude"
    ].between(40.5, 41.2)
    dataframe = dataframe[idx].copy()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--min_price", type=int, help="Minimum price limit", required=True
    )

    parser.add_argument(
        "--max_price", type=int, help="Maximum price limit", required=True
    )
